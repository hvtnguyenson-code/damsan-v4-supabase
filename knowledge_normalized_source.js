// 038/039C — AI Web -> DAMSAN_SOURCE_V2 normalized-source workflow.
// Grade, source role, and authority slot are explicit authoritative teacher selections.
(function () {
  'use strict';
  ...
  async function loadAuthoritySlots() {
    ...
    const docId = document.getElementById('normalizedDocument')?.value || '';
    const doc = normalizedDocuments.find((item) => item.id === docId);
    const grade = Number(document.getElementById('normalizedGrade')?.value || 0);
    if (!active || !doc || !doc.mon_id || ![10, 11, 12].includes(grade)) {
      select.innerHTML = '<option value="">-- Chọn tài liệu và khối trước --</option>';
      return;
    }
    ...
    try {
      ...
      if (!authoritySlots.length) {
        setStatus(`Không có authority slot ${role} phù hợp cho khối ${grade}. Không thể gắn tài liệu này vào căn cứ ra đề.`, 'error');
      } else {
  ...
  async function preparePrompt() {
    try {
      ...
      setBusy(true);
      setStatus('Đang khóa metadata và dựng prompt DAMSAN_SOURCE_V2...', 'info');
      ...
      const authorityText = isAuthorityRole(payload.source_role)
        ...
        : '';
      setStatus(`Prompt đã khóa metadata: khối ${payload.grade} · ${payload.source_role}${authorityText}. Hãy mở ChatGPT/Gemini, đính kèm đúng file gốc và yêu cầu AI tạo JSONL.`, 'ok');
    } catch (error) {
  ...
  async function importJsonl() {
    try {
      ...
      if (preparedDocumentId && (
        ...
        throw new Error('Tài liệu/khối/vai trò/căn cứ đích đã thay đổi sau khi tạo prompt. Hãy tạo lại prompt trước khi nhập JSONL để tránh gắn sai nguồn.');
      }
      ...
      setBusy(true);
      setStatus(`Đang khóa lại metadata và kiểm định ${file.name}...`, 'info');
      await lockMetadata(payload);
      ...
      setStatus(`Nhập thành công ${result.unit_count} units · ${activeText} · khối ${result.grade} · ${result.source_role}${bindingText}.`, result.active_revision ? 'ok' : 'info');
  ...
  function installUi() {
    ...
    section.innerHTML = `
      ...
      <div id="normalizedAuthorityWrap" class="hint-row hidden">
        ...
        <select id="normalizedAuthoritySlot"><option value="">-- Chọn tài liệu và khối trước --</option></select>
      ...
      <div class="hint-row"><label for="normalizedPrompt">Prompt gửi cho AI web</label><textarea id="normalizedPrompt" class="normalized-prompt" readonly placeholder="Chọn tài liệu + khối + vai trò nguồn; với Authority/Benchmark chọn thêm căn cứ đích, rồi bấm Tạo prompt chuẩn hóa..."></textarea></div>
      <div class="normalized-import">
        <div><strong>Bước nhập lại hệ thống</strong><div class="help">Tải file JSONL/NDJSON AI tạo. Server sẽ kiểm schema, đủ trang, unit_key, ranh giới bài, khối, vai trò nguồn, provenance; authority binding lấy từ lựa chọn giáo viên chứ không lấy từ AI.</div></div>
