...
async function generate(req:Request,actor:Actor,body:Obj){
  ...
  const request=await loadRequest(actor,requestId);const{provider,model}=await loadProviderModel(actor,providerId,modelProfileId);const params=mergeParameters(model,body);const promptHash=await sha256Hex(prompt);
