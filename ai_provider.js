...
function setBusy(v){aipBusy=v;for(const id of ['btnNew','btnRefresh','btnQuickConnect','btnSave','btnTest','btnDelete','btnCancel','btnAddModel']){const e=document.getElementById(id);if(e)e.disabled=v;}}
...
async function gateway(payload){
  ...
  if(!r.ok||!data||data.status!=='success'){const e=new Error(data?.message||data?.code||`Dịch vụ AI trả mã ${r.status}`);e.code=data?.code||'provider_control_failed';throw e;}return data;
}
function currentProvider(){return aipProviders.find(p=>p.id===aipCurrentId)||null;}
function testBadge(p){if(p.last_test_status==='OK')return '<span class="pill ok">✓ Sẵn sàng</span>';if(p.last_test_status==='FAILED')return '<span class="pill bad">Cần kiểm tra</span>';return '<span class="pill">Chưa kiểm tra</span>';}
function renderProviders(){
  const box=document.getElementById('providerList');
  if(!aipProviders.length){box.innerHTML='<div class="provider">Chưa có kết nối AI nào.</div>';return;}
  box.innerHTML=aipProviders.map(p=>`<div class="provider ${p.id===aipCurrentId?'active':''}" data-provider-id="${esc(p.id)}"><div class="provider-head"><div><strong>${esc(p.display_name)}</strong><div class="meta">${p.models?.length||0} model${p.secret_hint?` · key ${esc(p.secret_hint)}`:''}</div></div>${testBadge(p)}</div></div>`).join('');
...
function renderModels(){
  ...
  box.innerHTML=models.length?models.map(m=>`<div class="model"><div><strong>${esc(m.display_name||m.model_id)}</strong><div class="small">${esc(m.model_id)}</div></div><div class="actions" style="margin-top:0"><button class="secondary" data-model-edit="${esc(m.id)}">Sửa</button><button class="danger" data-model-delete="${esc(m.id)}">Xóa</button></div></div>`).join(''):'<div class="small">Chưa dò được model. Mở mục “Không thấy model?” bên dưới và nhập Model ID một lần.</div>';
...
function presetForProvider(p){
  if(!p)return'OPENAI_COMPAT';
  if(p.adapter_type==='GEMINI_GENERATE_CONTENT')return'GEMINI';
  if(p.adapter_type==='ANTHROPIC_MESSAGES')return'ANTHROPIC';
  if(p.adapter_type==='CUSTOM_JSON_HTTP')return'CUSTOM';
  if((p.adapter_type==='OPENAI_RESPONSES'||p.adapter_type==='OPENAI_CHAT')&&String(p.base_url||'').includes('api.openai.com'))return'OPENAI';
  return'OPENAI_COMPAT';
}
function applyPreset(preset,forceName=false){
  const base=document.getElementById('baseUrl'),adapter=document.getElementById('adapterType'),auth=document.getElementById('authType'),header=document.getElementById('authHeader'),prefix=document.getElementById('authPrefix'),modelsPath=document.getElementById('modelsPath'),discovery=document.getElementById('discoveryMode'),name=document.getElementById('displayName'),advanced=document.getElementById('advancedProviderSettings');
  discovery.value='HYBRID';modelsPath.value='models';
  if(preset==='OPENAI'){
    adapter.value='OPENAI_RESPONSES';base.value='https://api.openai.com/v1';auth.value='BEARER';header.value='';prefix.value='Bearer ';if(forceName||!name.value)name.value='OpenAI';
  }else if(preset==='GEMINI'){
    adapter.value='GEMINI_GENERATE_CONTENT';base.value='https://generativelanguage.googleapis.com/v1beta';auth.value='QUERY';header.value='key';prefix.value='';if(forceName||!name.value)name.value='Google Gemini';
  }else if(preset==='ANTHROPIC'){
    adapter.value='ANTHROPIC_MESSAGES';base.value='https://api.anthropic.com/v1';auth.value='HEADER';header.value='x-api-key';prefix.value='';if(forceName||!name.value)name.value='Anthropic Claude';
  }else if(preset==='CUSTOM'){
    adapter.value='CUSTOM_JSON_HTTP';auth.value='BEARER';header.value='';prefix.value='Bearer ';modelsPath.value='';if(forceName||!name.value)name.value='API tùy chỉnh';if(advanced)advanced.open=true;
  }else{
    adapter.value='OPENAI_COMPAT';auth.value='BEARER';header.value='';prefix.value='Bearer ';if(forceName||!name.value)name.value='API trung gian';if(forceName)base.value='';
  }
}
function blankEditor(){
  aipCurrentId='';document.getElementById('editorTitle').textContent='Thêm kết nối';
  document.getElementById('ownerScope').value='PERSONAL';document.getElementById('quickPreset').value='OPENAI_COMPAT';document.getElementById('displayName').value='';document.getElementById('adapterType').value='OPENAI_COMPAT';document.getElementById('baseUrl').value='';document.getElementById('discoveryMode').value='HYBRID';document.getElementById('authType').value='BEARER';document.getElementById('authHeader').value='';document.getElementById('authPrefix').value='Bearer ';document.getElementById('apiKey').value='';document.getElementById('apiKey').placeholder='Nhập API key';document.getElementById('modelsPath').value='models';document.getElementById('providerOptions').value='';document.getElementById('enabled').checked=true;document.getElementById('editorStatus').textContent='';document.getElementById('btnDelete').classList.add('hidden');document.getElementById('advancedProviderSettings').open=false;
  applyPreset('OPENAI_COMPAT',true);document.getElementById('editorCard').classList.remove('hidden');renderProviders();renderModels();
}
function openProvider(id){
  const p=aipProviders.find(x=>x.id===id);if(!p)return;aipCurrentId=id;
  document.getElementById('editorTitle').textContent=`Sửa: ${p.display_name}`;document.getElementById('quickPreset').value=presetForProvider(p);document.getElementById('ownerScope').value=p.owner_scope;document.getElementById('displayName').value=p.display_name||'';document.getElementById('adapterType').value=p.adapter_type;document.getElementById('baseUrl').value=p.base_url||'';document.getElementById('discoveryMode').value=p.discovery_mode||'HYBRID';document.getElementById('authType').value=p.auth_type||'BEARER';document.getElementById('authHeader').value=p.auth_header||'';document.getElementById('authPrefix').value=p.auth_prefix||'';document.getElementById('apiKey').value='';document.getElementById('apiKey').placeholder=p.secret_hint?`Đã lưu ${p.secret_hint}; để trống để giữ nguyên`:'Nhập API key';document.getElementById('modelsPath').value=p.models_path||'';document.getElementById('providerOptions').value=pretty(p.provider_options||{});document.getElementById('enabled').checked=p.enabled!==false;document.getElementById('editorStatus').textContent=p.last_test_message||'';document.getElementById('btnDelete').classList.remove('hidden');document.getElementById('advancedProviderSettings').open=false;document.getElementById('editorCard').classList.remove('hidden');renderProviders();renderModels();
}
...
async function quickConnect(){
  const saved=await gateway(providerPayload());aipCurrentId=saved.provider_id;document.getElementById('apiKey').value='';
  let testMessage='Đã lưu kết nối.';let verified=false;
  try{const tested=await gateway({action:'test_provider',provider_id:aipCurrentId});testMessage=tested.message||'Đã kiểm tra kết nối.';verified=!!tested.verified;}catch(e){testMessage='Đã lưu kết nối nhưng chưa dò được model. Có thể nhập Model ID thủ công bên dưới.';}
  await loadProviders();openProvider(aipCurrentId);
  const p=currentProvider();
  if(p?.models?.length)notice(`Xong. ${p.models.length} model đã sẵn sàng để tạo đề.`,'ok');
  else notice(testMessage,verified?'ok':'info');
}
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  document.getElementById('quickPreset').addEventListener('change',()=>applyPreset(document.getElementById('quickPreset').value,true));
  document.getElementById('btnNew').addEventListener('click',blankEditor);
  document.getElementById('btnRefresh').addEventListener('click',()=>run(async()=>{await loadProviders();notice('Đã làm mới danh sách kết nối.','info');}));
  document.getElementById('btnCancel').addEventListener('click',()=>document.getElementById('editorCard').classList.add('hidden'));
  document.getElementById('btnQuickConnect').addEventListener('click',()=>run(quickConnect));
  document.getElementById('btnSave').addEventListener('click',()=>run(async()=>{const data=await gateway(providerPayload());aipCurrentId=data.provider_id;document.getElementById('apiKey').value='';await loadProviders();openProvider(aipCurrentId);notice('Đã lưu cấu hình.','ok');}));
  document.getElementById('btnTest').addEventListener('click',()=>run(async()=>{if(!aipCurrentId)throw new Error('Hãy lưu kết nối trước khi kiểm tra.');const data=await gateway({action:'test_provider',provider_id:aipCurrentId});await loadProviders();openProvider(aipCurrentId);notice(data.message||'Đã kiểm tra kết nối.',data.verified?'ok':'info');}));
  document.getElementById('btnDelete').addEventListener('click',()=>run(async()=>{if(!aipCurrentId||!confirm('Xóa kết nối, model và API key đã lưu?'))return;await gateway({action:'delete_provider',provider_id:aipCurrentId});aipCurrentId='';document.getElementById('editorCard').classList.add('hidden');await loadProviders();notice('Đã xóa kết nối.','ok');}));
  document.getElementById('btnAddModel').addEventListener('click',()=>run(async()=>{
    if(!aipCurrentId)throw new Error('Chưa chọn kết nối.');
    ...
    document.getElementById('manualModelId').value='';document.getElementById('manualModelName').value='';document.getElementById('modelCapabilities').value='';document.getElementById('modelParameters').value='';await loadProviders();openProvider(aipCurrentId);notice('Đã thêm model.','ok');
  }));
  run(async()=>{await loadProviders();notice('Chỉ cần thêm một kết nối AI là có thể dùng API để tạo đề.','info');});
});