...
function blankEditor(){
  ...
  document.getElementById('ownerScope').value='PERSONAL';document.getElementById('displayName').value='';document.getElementById('adapterType').value='OPENAI_COMPAT';document.getElementById('baseUrl').value='';document.getElementById('discoveryMode').value='HYBRID';document.getElementById('authType').value='BEARER';document.getElementById('authHeader').value='';document.getElementById('authPrefix').value='';document.getElementById('apiKey').value='';document.getElementById('modelsPath').value='';document.getElementById('enabled').checked=true;document.getElementById('editorStatus').textContent='';document.getElementById('btnDelete').classList.add('hidden');document.getElementById('btnTest').classList.add('hidden');
...
function adapterDefaults(){
  const a=document.getElementById('adapterType').value;const auth=document.getElementById('authType'),base=document.getElementById('baseUrl'),header=document.getElementById('authHeader'),prefix=document.getElementById('authPrefix');
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  document.getElementById('btnAddModel').addEventListener('click',()=>run(async()=>{if(!aipCurrentId)throw new Error('Chưa chọn provider.');const modelId=document.getElementById('manualModelId').value.trim();if(!modelId)throw new Error('Cần nhập Model ID.');await gateway({action:'save_model',provider_id:aipCurrentId,model_id:modelId,display_name:document.getElementById('manualModelName').value.trim()||modelId,capabilities:{},default_parameters:{},enabled:true});document.getElementById('manualModelId').value='';document.getElementById('manualModelName').value='';await loadProviders();openProvider(aipCurrentId);notice('Đã thêm model.','ok');}));
  run(async()=>{await loadProviders();notice('Đã tải Provider Control Plane 065A.','info');});
});
