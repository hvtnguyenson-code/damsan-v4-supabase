...
const boundarySemanticsPath = path.join(root, 'supabase', 'migrations', '20260910055318_knowledge_reader_doc_boundary_semantics_030b2.sql');
...
for (const file of [migrationPath, queueSafetyPath, boundarySemanticsPath, edgePath, htmlPath, baseJsPath, readerPath]) {
...
const boundarySql = fs.readFileSync(boundarySemanticsPath, 'utf8');
...
must(boundarySql, /v_physical_page_count := case when v_boundary_mode = 'PDF_PAGE' then v_boundary_count else null end/i,
  'K030B2-13 only PDF_PAGE boundaries become physical page_count');
must(boundarySql, /page_count = coalesce\(v_physical_page_count, page_count\)/i,
  'K030B2-14 DOC and DOCX do not fabricate a physical page count');
console.log('K030B2-11..14 queue and boundary semantics: PASSED');

must(edge, /SUPABASE_SERVICE_ROLE_KEY/, 'K030B2-15 edge uses server service credential');
assert(
  ...
  'K030B2-16 edge validates opaque staff session hash, revocation and expiry'
);
must(edge, /document\.owner_gv_id !== actor\.id \|\| document\.truong_id !== actor\.truong_id/i,
  'K030B2-17 edge binds extraction to owned document');
must(edge, /createSignedUploadUrl\(storagePath\)/i, 'K030B2-18 artifact uses signed upload capability');
must(edge, /action === "prepare_artifact"/i, 'K030B2-19 prepare action exists');
must(edge, /action === "complete_artifact"/i, 'K030B2-20 complete action exists');
must(edge, /rpc_knowledge_commit_extraction_service/i, 'K030B2-21 edge commits through service RPC');
must(edge, /artifactPrefix\(actor, owned\)/i, 'K030B2-22 storage path is actor/document/job scoped');
console.log('K030B2-15..22 secure extraction gateway: PASSED');

must(html, /worker-src 'self' blob: https:\/\/cdn\.jsdelivr\.net/i,
  'K030B2-23 CSP allows only declared OCR/PDF worker source');
must(html, /connect-src[^;]*https:\/\/tessdata\.projectnaptha\.com/i,
  'K030B2-24 CSP allows OCR language data source');
must(html, /mammoth@1\.12\.2\/mammoth\.browser\.min\.js/i, 'K030B2-25 Mammoth is pinned');
must(html, /tesseract\.js@7\.0\.0\/dist\/tesseract\.min\.js/i, 'K030B2-26 Tesseract is pinned');
must(html, /Alpaq92\/JSDoc@821695a884e0c0bb8592a635d9524bb3e116cd67\/src\/docToText\.js/i,
  'K030B2-27 legacy DOC parser is pinned to exact reviewed revision');
must(html, /knowledge_reader\.js\?v=20260910-knowledge-reader-030b2/i,
  'K030B2-28 automatic reader is loaded after upload base');
console.log('K030B2-23..28 browser dependency/CSP contract: PASSED');

must(reader, /pdfjs-dist@\$\{KNOWLEDGE_PDFJS_VERSION\}\/build\/pdf\.mjs/i,
  'K030B2-29 PDF.js module extraction is used');
must(reader, /pdf\.worker\.mjs/i, 'K030B2-30 PDF.js worker is configured');
must(reader, /mammoth\.extractRawText\(\{ arrayBuffer:/i, 'K030B2-31 DOCX extraction uses Mammoth');
must(reader, /window\.docToText\(await file\.arrayBuffer\(\)\)/i, 'K030B2-32 legacy DOC extraction is automatic');
must(reader, /createWorker\('vie\+eng'/i, 'K030B2-33 OCR is Vietnamese + English');
must(reader, /if \(knowledgeReaderNeedsOcr\(text\)\) sparsePages\.push\(pageNumber\)/i,
  'K030B2-34 OCR is selective, not applied blindly to every PDF page');
must(reader, /DAMSAN_EXTRACT_V1/i, 'K030B2-35 extraction artifact is canonical');
must(reader, /uploadToSignedUrl\(prepared\.storage_path, prepared\.upload_token, artifactBlob/i,
  'K030B2-36 full extracted text goes directly to private Storage');
must(reader, /knowledgeUploadSelected = async function knowledgeUploadSelected030B2/i,
  'K030B2-37 upload action automatically continues into document reading');
must(reader, /pipeline_status \|\| 'EXTRACTED'/i, 'K030B2-38 UI reports extracted state');
console.log('K030B2-29..38 automatic reader pipeline: PASSED');

assert(!/SUPABASE_SERVICE_ROLE_KEY/.test(baseJs), 'K030B2-39 base browser code has no service-role key');
assert(!/SUPABASE_SERVICE_ROLE_KEY/.test(reader), 'K030B2-40 reader browser code has no service-role key');
assert(!/service_role/i.test(reader), 'K030B2-41 browser reader does not assume service-role access');
console.log('K030B2-39..41 browser credential isolation: PASSED');
